  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://the-met-gallery-backend.onrender.com/login",
        {
          method: "POST",
          body: JSON.stringify(formData),
          headers: {
      ...
      if (!response.ok) {
        ...
        if (data.message === "Please verify your email before logging in.") {
          ...
          const redirectPath =
            ...
            (user.role === "admin" ? "/dashboard/overview" : "/home");

          localStorage.removeItem("redirectAfterLogin");
          
          setTimeout(() => navigate(redirectPath), 1500); // Delay redirect by 1.5 seconds to show the message
        }
      }
    } catch (err) {
      console.log("Fetch Error:", err); // Log any fetch errors
  ...
  return (
    <div className="login-container">
      ...
      <form onSubmit={handleSubmit} className="login-form">
        ...
        <button
          type="submit"
          className="login-button" // Use CSS class for styling
        >
          Login
        </button>
      </form>
